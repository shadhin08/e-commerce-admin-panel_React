...
const Vendors = () => {
    ...
    const { isLoading, data: vendors = [], refetch } = useQuery(
    {
        queryKey: [],
        queryFn: () => axios.get(`${import.meta.env.VITE_API_URL}/vendor/get-all-vendor`,
            {
                headers: {
    ...
    return (
        <div>
            <Helmet>
                <title>Vendors - Ekka Dashboard</title>
                        ...
                        <table id="brandsTable" className="table table-striped table-bordered border-top border-bottom">
                            ...
                            <tbody>
                                ...
                                {vendors?.map((vendor, index) =>
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td className='text-capitalize'>{vendor?.name}</td>
                                        <td className='text-capitalize'>{vendor?.contact_no? vendor?.contact_no : 'Not Added'}</td>
                                        <td className='text-capitalize'>{vendor?.email ? vendor?.email : 'Not Added'}</td>
                                        <td className='text-capitalize'>{vendor?.address}</td>
                                        <td>
                                            <button type="button" className="btn btn-outline-primary btn-sm me-1"><i className='bx bx-receipt'></i>Details</button>
                                            <Link to={`/dashboard/vendor/setting/${vendor?._id}`} type="button" className="btn btn-outline-primary btn-sm"><i className='bx bx-slider'></i>Setting</Link>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
